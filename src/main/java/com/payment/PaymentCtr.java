...
public class PaymentCtr<Payment> {
	...
	public String Payment_page(HttpServletResponse response, HttpServletRequest request, HttpSession session, ModelMap modelMap) throws IOException {
		...
		modelMap.addAttribute("customer_uid", id);
		
		return "view/payment";
	}
	
	@RequestMapping(value = "insert_payment", method = RequestMethod.POST)
	public String insert_payment(HttpServletRequest request, ModelMap modelMap, PaymentVO vo, 
			HttpSession session, HttpServletResponse response) throws IOException, InterruptedException {
		...
		service.insert_payment(vo);
		
		ScriptUtils.alert(response, "예매가 완료되었습니다.");
